...
function ensureMeetingPath({ objectPath, moduleName, meetingId }) {
  const prefix = `${moduleName}/${meetingId}/`;
...
export async function POST(req, { params }) {
  ...
  if (getFirebaseAdminInitError() || !storageBucket) {
    return NextResponse.json(
      { success: false, message: "Monthly meeting upload is not configured." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    const meetingId = sanitizeSegment(resolvedParams?.id);
    const formData = await req.formData();
    const file = formData.get("file");
    const moduleName = sanitizeSegment(formData.get("module"));
    const folder = sanitizeSegment(formData.get("folder"));
    ...
    const prefix = folder ? `${moduleName}/${meetingId}/${folder}` : `${moduleName}/${meetingId}`;
    const objectPath = `${prefix}/${nonce}_${fileName}`;
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    if (!meetingId || !moduleName || !objectPath) {
      return NextResponse.json({ success: false, message: "Missing delete payload." }, { status: 400 });
    }
    if (!ALLOWED_MODULES.has(moduleName)) {
      return NextResponse.json({ success: false, message: "Invalid module." }, { status: 400 });
    }
    if (!ensureMeetingPath({ objectPath, moduleName, meetingId })) {
